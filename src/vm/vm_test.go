...
func testExpectedObject(
	...
	case map[object.HashKey]int64: // FIXME 这么写不通用，要优化
		...
		for ek, ev := range expected {
			pair, ok := hash.Pairs[ek]
			if !ok {
				t.Errorf("no pair for given key in pairs")
			}

			err := testIntegerObject(ev, pair.Value)
			if err != nil {
				t.Errorf("testIntegerObject failed: %s", err)
			}
		}
...
func testStringObject(expected string, actual object.Object) error {
	...
	if !ok {
		return fmt.Errorf("object is not String. got=%T(%+v)", strObj, strObj)
	}

	if strObj.Value != expected {
		return fmt.Errorf("object has wrong value. got=%q, want=%q", strObj.Value, expected)
	}
	return nil
}
...
func testBooleanObject(expected bool, actual object.Object) error {
	...
	if result.Value != expected {
		return fmt.Errorf("obejct has wrong value.got=%t, want=%t", result.Value, expected)
	}
	return nil
}
